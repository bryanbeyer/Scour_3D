...
Lx_p = 8;  %Length of channel
Ly_p = 1;  %Width of channel
Lz_p = 2;  %Height of channel

%Pile occupies full height of channel
X_c_p = Lx_p/2; %Location of pile centroid
Y_c_p = Ly_p/2; %Center of channel
R_c_p = 0.1*Ly_p; %radius of pile
...
% zy_p face.  Bottom will be solid up to 0.2*Lz_p
bottom_height = 0.2*Lz_p;
bottom = find(gcoord(:,3) =< bottom_height);

%Find bed boundary layer
bed = bottom + Nx*Ny;
bed = setxor(bed,intersect(bed,bottom);
bed = setxor(bed,intersect(bed,circ_list);

snl = union(bottom, circ_list);
snl = unique(snl);

%Set inlet/outlet nodes.
inl = faces.zy_m; 
...
ux_p_in = Umax*(1 - ((gcoord(inl,2)-by)/by).^2).*(1-((gcoord(inl,3)-bz)/bz).^2);
ux_p_out = Umax*(1 - ((gcoord(onl,2)-by)/by).^2).*(1-((gcoord(onl,3)-bz)/bz).^2);

uy_p_in = zeros(length(ux_p_in),1);
uz_p_in = uy_p_in;

uy_p_out = zeros(length(ux_p_out),1);
uz_p_out = uy_p_out;

%Shear Properties
D_50 = 0.00025; %m, fine river sand
K_u = 6.19; %m^0.5/s, universal constand
depth = Lz - bottom_height; %water depth equal to hydraulic radius

u_cr = K_u*depth^(1/6)*D_50^(1/3);
...
%            pause(0.05);
    ...
    if(ts >= ts_shear)
       ...
       u_bed = sqrt(ux.^2 + uy.^2 + uz.^2);
       u_exceed = find(u_bed)=>u_cr;
       scour_area = intersect(u_exceed,bed);
       transport = scour_area - Nx*Ny;
       bottom = setxor(bottom,intersect(bottom,transport));
